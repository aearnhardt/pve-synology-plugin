...
restart_pve_stack() {
  ...
  # restart (not try-restart) so daemons reload Custom/*.pm even if they were stopped
  "${cmd[@]}" restart pve-cluster.service 2>/dev/null || true
  "${cmd[@]}" restart pvedaemon.service 2>/dev/null || true
  "${cmd[@]}" restart pvestatd.service 2>/dev/null || true
  "${cmd[@]}" restart pveproxy.service 2>/dev/null || true
  "${cmd[@]}" restart pvescheduler.service 2>/dev/null || true
}
...
plugin_load_hint() {
  cat <<'EOF'
The plugin .pm file is on disk, but Proxmox did not register storage type "synology".
Without that, pvesm add synology will fail (unknown storage type or missing plugin options).

On the PVE node (as root):
  journalctl -b --no-pager | grep -iE 'SynologyStoragePlugin|storage plugin'
  perl -MJSON::XS -MLWP::UserAgent -e 1
  systemctl restart pvedaemon pvestatd pveproxy
  perl -e 'use PVE::Storage; use PVE::Storage::Plugin; print join("\n", PVE::Storage::Plugin->lookup_types()), "\n";' | grep synology

Re-run install.sh menu option 1 (Install plugin), then option 2 (Configure storage).
EOF
}
...
ensure_plugin_registered() {
  ...
  echo "$SCRIPT_NAME: Synology storage type is not registered in pvesm." >&2
  plugin_load_hint >&2
  return 1
}
...
install_plugin() {
  ...
  if [[ -d /run/systemd/system ]]; then
    restart_pve_stack
    info "Restarted pve-cluster, pvedaemon, pvestatd, pveproxy, pvescheduler."
  fi

  if ! synology_plugin_registered; then
    info "Warning: plugin file installed but storage type 'synology' is not registered yet."
    plugin_load_hint
  fi
}
...
configure_storage() {
  ...
  ensure_plugin_registered || die "cannot configure storage until the plugin is loaded (see messages above)"
  ...
  if prompt_yesno "Also store LXC container roots on this storage (rootdir)?" y; then
    content=$(append_csv_unique "$content" "rootdir")
  fi
  ...
  # pvesm uses underscores in flag names (storage.cfg keys), not hyphens: --target_name not --target-name
  ...
  if [[ $pvesm_rc -ne 0 ]]; then
    if ! synology_plugin_registered; then
      die "pvesm add failed (exit $pvesm_rc). The synology plugin is not loaded — install/restart first (see messages above)."
    fi
    die "pvesm add failed (exit $pvesm_rc)"
  fi
